...
if (process.env.BOOSTO) { plugin_paths.unshift(path.join(process.env.BOOSTO, 'plugins')); }
...
plugins._load_and_compile_plugin = function(name) {
    ...
    var custom_require = function (module) {
        if (!/^\./.test(module)) {
    ...
    try {
        ...
        throw err; // default is to re-throw and stop
    }
