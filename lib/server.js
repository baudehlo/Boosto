...
var defaults = {
    ...
    daemonize: false,
    daemon_log_file: '/var/log/boosto.log',
    daemon_pid_file: '/var/run/boosto.pid'
};
...
Server.createServer = function (params) {
    var config_data = config.get('imap.ini', {booleans: ['daemonize']});
    var param_key;
...
function setup_listeners (listeners, plugins, type, inactivity_timeout) {
    async.each(listeners, function (host_port, cb) {
        ...
        if (!hp) {
            return cb(new Error("Invalid format for listen parameter in imap.ini"));
        }
        ...
        if (hp[2] == 993) {
            ...
            if (!options.key) {
                return cb(new Error("Missing tls_key.pem for port 993"));
            }
            if (!options.cert) {
                return cb(new Error("Missing tls_cert.pem for port 993"));
            }
